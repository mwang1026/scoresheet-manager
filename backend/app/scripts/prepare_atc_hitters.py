"""Match ATC hitter names to mlb_ids and output enriched TSV.

Reads a raw ATC hitter TSV (from FanGraphs/ATC projection export) and
...
Usage:
    cd backend
    python -m app.scripts.prepare_atc_hitters \
        "../data/ATC 2026 - Hitting - raw.tsv" \
        "../data/ATC 2026 - Hitting.tsv"
"""
...
def prepare_atc_hitters(input_path: str, output_path: str) -> None:
    ...
    try:
        ...
        with open(input_path, "r", encoding="utf-8") as fin, \
            ...
            for row in reader:
                ...
                scoresheet_team = TEAM_ABBR_MAP.get(atc_team)
                ...
                if override_db_name:
                    ...
                    mlb_id = match_player(first, last, scoresheet_team, exact_lookup, name_lookup)
                else:
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Usage: python -m app.scripts.prepare_atc_hitters <input_tsv> <output_tsv>")
        sys.exit(1)
    ...
    prepare_atc_hitters(input_path, output_path)
