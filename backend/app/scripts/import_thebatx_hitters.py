"""Import TheBatX hitter projections from enriched TSV file.

Reads a TSV file with an mlb_id column (output of prepare_atc_hitters.py)
and upserts HitterProjection records with source="TheBatX".

Uses the same FanGraphs column layout as ATC projections.

Usage:
    cd backend
    python -m app.scripts.import_thebatx_hitters "../data/TheBatX 2026 - Hitting.tsv"
"""
...
def import_thebatx_hitters(tsv_path: str) -> None:
    """Import TheBatX hitter projections from enriched TSV file."""
    db = SessionLocal()
    ...
    try:
        with open(tsv_path, "r", encoding="utf-8") as f:
            ...
            for row in reader:
                ...
                projection_data = parse_atc_hitter_projection(row, player.id, source="TheBatX")
                ...
                if projections_imported % 100 == 0:
                    logger.info("Imported %d TheBatX hitter projections...", projections_imported)
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python -m app.scripts.import_thebatx_hitters <path_to_tsv>")
        sys.exit(1)
    ...
    import_thebatx_hitters(tsv_path)
