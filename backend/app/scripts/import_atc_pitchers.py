"""Import ATC pitcher projections from enriched TSV file.

Reads a TSV file with an mlb_id column (output of prepare_atc_pitchers.py)
and upserts PitcherProjection records with source="ATC".
...
Usage:
    cd backend
    python -m app.scripts.import_atc_pitchers "../data/ATC 2026 - Pitching.tsv"
"""
...
def import_atc_pitchers(tsv_path: str) -> None:
    """Import ATC pitcher projections from enriched TSV file."""
    db = SessionLocal()
    ...
    try:
        with open(tsv_path, "r", encoding="utf-8") as f:
            ...
            for row in reader:
                ...
                projection_data = parse_atc_pitcher_projection(row, player.id)
                ...
                if projections_imported % 100 == 0:
                    logger.info("Imported %d ATC pitcher projections...", projections_imported)
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python -m app.scripts.import_atc_pitchers <path_to_tsv>")
        sys.exit(1)
